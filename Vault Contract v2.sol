...
contract VaultContract is Ownable{
    ...
    */
     ...
     * @param _public The new address where respective part of crowdsale tokens can be transferred for distribution
    */
    function setPublicAddress(address _public) public onlyOwner{
        public_ = _public;
    }
    ...
    */ 
    function getPublicAvailable() public view returns(uint256){
        return getPublicReleasable().sub(alreadyWithdrawn[publicKey]);
    }    
    ...
    */
    ...
    // PUBLIC TOKENSALE; 4.25% for public sale; available right away
    function getPublicReleasable() public view returns(uint256){
        return token.totalSupply().div(10000).mul(425);
    }
    ...
    function getLiquidityReleasable() public view returns(uint256){
        if(block.timestamp >= TGE_timestamp){
            return token.totalSupply().div(10000).mul(575);
        }else{
            return 0;
        }
    }
     ...
     */ 
    function releasePublic() public{
        require(token.balanceOf(address(this))>=getEcosystemAvailable(),'Vault does not have enough tokens');
        uint256 toSend = getPublicAvailable();
        incrementReleased(publicKey,toSend);
        require(token.transfer(public_, toSend),'Token Transfer returned false');
    }
